...
public class AndroidBarcodeView extends View {
    String mys1 = new String("123456790");
    int myn1 = 230;

    public AndroidBarcodeView(Context context, String s1, int n1) {
        super(context);
        mys1 = s1;
        myn1 = n1;
        GQLog.d("barcodeview", "width = " + n1);
    }
    ...
    static String targetString = new String("0123456789");
    static int index = 230;

    private static void testCODE39(Canvas canvas, String target, int width) throws Exception {
        index = (width * 230) / 768; // see below some sort of pixes to points conversion should be center if different widths.
        targetString = target;
        testCODE39(canvas);
    }
    ...
    private static void testCODE39(Canvas canvas) throws Exception {
	    ...
	    */
        //int index = 230;
        RectF bounds = new RectF(30 + index, 30, 0 + index, 0);
        barcode.drawBarcode(canvas, bounds);
    }
